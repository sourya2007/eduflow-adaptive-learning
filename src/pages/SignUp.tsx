...
export default function SignUp() {
  ...
  const handleGoogle = async () => {
    setError('');
    const fakeCred = JSON.stringify({ email: 'google.user@example.com', name: 'Google User', sub: 'g002' });
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4 md:p-8 font-sans">
      <div className="w-full bg-surface rounded-2xl shadow-lg p-8 md:p-12 border border-surface-variant" style={{maxWidth:448}}>
        <div className="text-center mb-10">
          ...
          <p className="text-on-surface-variant mt-3 text-base md:text-lg">Create your account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <button type="submit" disabled={loading} className="w-full bg-primary hover:bg-primary/90 text-on-primary py-4 rounded-xl font-semibold shadow-sm transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-base md:text-lg">{loading ? 'Creating account...' : <><UserPlus className="w-5 h-5" /> Sign Up</>}</button>
        </form>
